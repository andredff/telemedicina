...
import type { Specialty } from "./SpecialtyCard";
...
export function ScheduleSpecialistModal({
  ...
  const getLoadingMessage = () => {
    switch (flowStep) {
      ...
      case "creating_consultation": return "Criando agendamento...";
  ...
  const stepIndex =
    currentStep === "specialty" ? 0
    : currentStep === "professional" ? 1
          ...
          {steps.map((s, i) => {
            ...
            return (
              <div key={s.label} className="flex items-center gap-1 flex-1">
                ...
                {i < steps.length - 1 && (
                  <div className={`flex-1 h-0.5 ${isCompleted ? "bg-primary/40" : "bg-border"}`} />
                )}
              </div>
            );
          })}
          ...
          <div className="space-y-3 py-2">
            {filteredSpecialties.length > 0 ? (
              filteredSpecialties.map((specialty) => (
                ...
                >
                  <CardContent className="p-4 flex items-center gap-3">
                    ...
                    {specialty.triagem && <Badge variant="outline" className="text-xs shrink-0">Triagem</Badge>}
                  </CardContent>
                </Card>
              ))
            ) : (
              <Alert>
                ...
                <AlertDescription>Não há especialidades disponíveis no momento.</AlertDescription>
              </Alert>
            )}
          </div>
        )}

        {!isLoading && flowStep !== "error" && currentStep === "professional" && flowStep === "selecting_professional" && (
          <div className="space-y-3 py-2">
            <div className="flex items-center gap-2 mb-2">
              ...
              {selectedSpecialty && <Badge variant="secondary">{selectedSpecialty.nome}</Badge>}
            </div>
            {availableProfessionals.length > 0 ? (
                ...
                >
                  <CardContent className="p-4 flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm text-muted-foreground">{professional.especialidadeNome}</p>
                    </div>
                  </CardContent>
                </Card>
              ))
              ...
              <Alert>
                ...
                <AlertDescription>Não há profissionais disponíveis para esta especialidade no momento.</AlertDescription>
              </Alert>
            )}
          </div>
        )}

        {!isLoading && flowStep !== "error" && currentStep === "schedule" && flowStep === "selecting_schedule" && (
          <div className="space-y-4 py-2">
            <div className="flex items-center gap-2 mb-2">
              <Button variant="ghost" size="sm" onClick={handleBack}>
                <ArrowLeft className="h-4 w-4 mr-1" />
              ...
              {selectedProfessional && <Badge variant="secondary">{selectedProfessional.nome}</Badge>}
            </div>
            {availableSchedules.length > 0 ? (
                    ...
                    {availableSchedules.map((day) => {
                      ...
                      return (
                        <Button
                          ...
                          size="sm"
                          onClick={() => { setSelectedDate(day.data); setSelectedSlot(null); }}
                          className="min-w-[100px]"
                        >
                          <div className="text-center">
                            <div className="text-xs">{format(dateObj, "EEE", { locale: ptBR })}</div>
                            <div className="font-semibold">{format(dateObj, "dd/MM", { locale: ptBR })}</div>
                          </div>
                        </Button>
                      );
                    })}
              ...
              <Alert>
                ...
                <AlertDescription>Não há horários disponíveis. Tente outro profissional ou volte mais tarde.</AlertDescription>
              </Alert>
            )}
          </div>
        )}
                    ...
                    <div>
                      ...
                      {selectedSlot && (
                        <p className="font-medium">
                          {format(new Date(selectedSlot.dataHora), "EEEE, dd 'de' MMMM 'de' yyyy 'às' HH:mm", { locale: ptBR })}
                        </p>
                      )}
                    </div>
                  </div>
